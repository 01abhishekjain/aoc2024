...
try {
  ...
  updates.forEach((update) => {
    ...
    for (i = 0; i < update.length; i++) {
      ...
      const firstBreakingRule = potentialBreakingRules.find(([n1]) =>
        update.slice(i + 1).includes(n1)
      );
      if (firstBreakingRule) {
        needsFix = true;
        swap(update, firstBreakingRule);
        i--;
        continue;
      }
    }
...
function swap(arr: string[], [p1, p2]: string[]) {
  let p1i = arr.indexOf(p1);
  let p2i = arr.indexOf(p2);
